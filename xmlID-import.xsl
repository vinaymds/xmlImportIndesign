...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="img" mode="images-byline">
    <xsl:call-template name="output-image">
      <xsl:with-param name="img-id" select="'byline'"/>
    </xsl:call-template>
  </xsl:template>

  <xsl:template match="img" mode="images-lead">
    <xsl:call-template name="output-image">
      <xsl:with-param name="img-id" select="'lead'"/>
    </xsl:call-template>
  </xsl:template>

  <xsl:template match="body/div/p/amedia-embed/img" mode="images-assets">
    <xsl:call-template name="output-image">
  ...
  <xsl:template match="p/italic | p/bold | li/italic | li/bold">
    ...
    <xsl:variable name="cont" select="name(ancestor::*[self::div or
                                                       self::body or
                                                       self::facts or
                                                       self::quote][1])"/>
  ...
  <xsl:template match="authors/name | authors/email"/>

  <xsl:template match="authors/name | authors/email" mode="bylines">
  ...
  <xsl:template match="headline | aHistorie/title |
                       leadText | lead/p |
                       maintag |
                       body | body/div/p | body/ul/li | body/ol/li |
  ...
  <xsl:template match="/*">
    <xsl:element name="{$prefix}">
      ...
      <!-- Byline elements -->
      <bylines>
        <xsl:apply-templates select="//authors/name"  mode="bylines"/>
        <xsl:apply-templates select="//authors/email" mode="bylines"/>
      </bylines>
      ...
      <xsl:apply-templates select="//body/div/p/amedia-embed/img"       mode="images-assets"/>
    </xsl:element>
  </xsl:template>
